...
def is_brandlike(t):
    """짧은 브랜드/워터마크성 '중국어' 글자인가(색상·사이즈·숫자 제외) — 교차이미지 반복 탐지용.
    라틴은 상품 그래픽 프린트('Hello.' 등)와 구분이 불가해 단독 탐지하지 않고,
    한자 워터마크에 인접한 경우(co-location)에만 제거한다."""
    s = t.strip()
    chars = re.findall(r"[一-鿿]", s)
    if not chars or not (2 <= len(chars) <= 6):
        return False
